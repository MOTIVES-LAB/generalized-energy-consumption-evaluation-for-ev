function data_summary = raw_summarize_elev(data_path,filenames,para,ind_CS,speed_CS,err,spd_lv,row,col,flag_showfig,ind_cltc,dur_sub_phase)
    ...
    str_results = ["cycle_name","speed_intensity","braking_intensity","braking_intensity_hi","braking_intensity_lo","slow_driving_intensity",...
                    "energy_consumption_Wh_km","p_idel_W","trip_distance_km","idle_duration_s","trip_consumption_Wh","trip_duration_h","loss_brake_hi","loss_brake_lo","diff_elev","ECR_chk_Wh_km"];

    
    % ------------number of cycles ------------- %
    num_cycle = length(filenames);
    prog_disp = floor(num_cycle/5):floor(num_cycle/5):num_cycle;
    
    % --------------------init-------------------%
    no_cycle = nan(num_cycle,1);
...
%         T = readtable(filename_cycle);
        warning on
        prog = find(prog_disp==i);
        if prog
            disp(strcat(num2str(ceil(i*100/num_cycle)),"% ……"));
        end
...
%         spd_profile = [t spd];
        ...
        if any("tot_char" == string(T.Properties.VariableNames)) && any("tot_disc" == string(T.Properties.VariableNames))
            E = (T.tot_disc(end)-T.tot_disc(1) - (T.tot_char(end)-T.tot_char(1)))*1000;
            ECR_chk = EC;
            EC = E/D;
        else
            ECR_chk = NaN;
...
%         ECR_chk = NaN;
        ...
        results_cycle(i,14) = diff_elev;
        results_cycle(i,15) = ECR_chk;
...
%% ====== deal with sub-pahse data ===== %%
    ...
    %             show_spd_pow(spd_sub,pow_sub,name_cycle((i-1)*3+j))
        ...
        data_summary = cell2table([ num2cell(no_cycle),num2cell(results_cycle);...
